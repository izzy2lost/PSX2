...
private:
	...
	// Sized at swapchain creation to image count + 2: the presentation engine
	// can hold a present-wait semaphore per swapchain image (min 6 on Mali),
	// so a fixed pool of NUM_SEMAPHORES recycles semaphores that may still be
	// in use by the presentation engine.
